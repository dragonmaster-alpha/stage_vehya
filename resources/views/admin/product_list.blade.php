          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                ...
                <a href="add-product" class="btn btn-success" style="float:right;">Add Product</a>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
                <table id="example1" class="table table-bordered table-striped">
                  ...
                  <tr>
                    ...
                    <td>{{$list['name']}}</td>
                    <td>{{$list['price']}}</td>
                    <td>{{$list['max_charge']}}</td>
                    <td>{{$list['voltage']}}</td>
                    <td>{{$list['warranty']}}</td>
                    <td><img src="{{env('APP_URL').'public/uploads/image/'}}{{$list['product_image']}}" width="50"></td>
                     <td>
                    <a href="product-list-view/{{$list['id']}}"><button class="btn btn-info"><i class="fa fa-eye" aria-hidden="true"></i></button></a>
                    <a href="product-list-show/{{$list['id']}}"><button class="btn btn-info"><i class="fas fa-edit"></i></button></a>
...
function a(id){
    var r = confirm("Are you sure ?");
         if (r == true) {
             location.replace("{{env('APP_URL')}}admin/product-list/"+id)
         } else {
            }
}
</script>
